...
md"""
Number of rows:
$(@bind number_of_rows Scrubbable(1:200))
"""

# ╔═╡ 72b4e48f-6700-4376-9548-f6523fc9c63f
md"""
Number of elements in row:
$(@bind number_of_ppr Scrubbable(1:200))
"""

# ╔═╡ 62aea3fe-8050-4817-9458-af98aeae88b6
md"""
Number of communities:
$(@bind number_of_communities Scrubbable(2:15))
"""

# ╔═╡ b50e46cb-3754-4ffe-bf27-7e3c54373be0
rows_per_communities = number_of_rows / number_of_communities

# ╔═╡ e3925cfa-9f1d-4f82-a27b-5e8129b2829b
matrix = [1 + floor((i - 1) / rows_per_communities) for i in 1:number_of_rows, j in 1:number_of_ppr]
...
function calculate_break_time!(mat, logs, m, time)
	if m <= number_of_people
		...
		same_class = vcat(people_groups .== group, timetable[Int64(ceil(time / 60))] .== 9999)
		mat[m, same_class] .+= 1
	else
		same_class = vcat(people_classes .== 9999, timetable[Int64(ceil(time / 60))] .!= 9999)
		mat[m, same_class] .+= 1
	end
end
...
# ╔═╡ b362c1c8-b587-44ca-a4a9-7039a2d3f7bf
	...
	if go
		for (i, g) in enumerate(graphs_logs)
			...
			open("logs/$(name).txt","w") do io
				...
				println(io, "Number of friend groups $(number_of_teachers)")
