...
# ╔═╡ 5c02e330-5f7a-4d48-94f6-39e049e1bf8d
begin
	using Graphs
...
begin
	...
	import .com_func_jl: relabel_communities, calculate_merging, calculate_splitting, calculate_unusual_appearance, calculate_community_activation, calculate_community_creation, calculate_community_deactivation, calculate_community_disappearance, calculate_unusual_size_change
end
...
function getTimestampVectorFromCommunityVector(c, sizeLists, f)
	n = maximum(maximum.(c))
	c_a = [f(sizeLists[i]) for i in 1:n]
...
function getMergedAtT(i, m_vec, t)
	x = findfirst(x -> fst(x) == t, m_vec)
	if x == nothing
		return
	end

	return (i, snd(m_vec[x]))
end

# ╔═╡ c607733c-bee4-4d63-8ce5-8ae59d6c7793
function getTimestampVectorFromMergeVector(c, sizeLists, f)
	l = length(sizeLists[1])
	n = maximum(maximum.(c))
	
	anomalies = [[] for i in 1:l]
	
	for i in 1:n
		merges = f(c, sizeLists, i)
		for j in 1:l
			mergeFound = getMergedAtT(i, merges, j)
			if mergeFound != nothing
				push!(anomalies[j], mergeFound)
			end
		end
	end
...
function getCommunityAnomaliesByTimestamp(g, c, sizeLists)
	CommunityAnomalies(
		...
		getTimestampVectorFromCommunityVector(c, sizeLists, calculate_community_creation),
		getTimestampVectorFromCommunityVector(c, sizeLists, calculate_community_disappearance),
		getTimestampVectorFromCommunityVector(c, sizeLists, calculate_unusual_size_change),
